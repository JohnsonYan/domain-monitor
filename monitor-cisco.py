...
class MultiThread(object):
    def __init__(self):
        ...
        self.count = 0
        # filename
        self.filename = './tools/cisco_without_alex.list'
    ...
    def schedule(self):
        ...
        if int(_time.split(':')[0]) in self.time_list:
            log.info('Task: monitor-cisco Started.')
            self.start_monitor()
            log.info('Task: monitor-cisco Completed.')
...
class DomainMonitor(threading.Thread):
    ...
    def monitor(self, domain):
        ...
        self.cisco.insert(self.document)
    ...
    def clean_outdated_domains(self):
        ...
        deleted_count = self.cisco.delete_many({'timestamp': {'$lt': _outdate}}).deleted_count
